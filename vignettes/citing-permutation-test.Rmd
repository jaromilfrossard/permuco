...
The `clusterlm(...)` function has several multiple comparison procedure implemented. It can be chosen using the `multcomp` argument. 

Moreover, the implemented method does not control, the same error rate/measure, we recall the measures as well as the reference in the following table:
...
`benjamini_hochberg`   | @benjamini_controlling_1995    |  |



For completeness and full reproducibility of the results, you can also report the parameters used in the `clustermass` and `tfce` method.
